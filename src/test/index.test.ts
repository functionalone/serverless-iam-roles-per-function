...
describe('plugin tests', function (this: any) {
  ...
  beforeEach(async () => {
    ...
    let compile_hooks: any[] = serverless.pluginManager.getHooks(
    ...
    for (const ent of compile_hooks) {
      try {
        ...
        console.log(
          'failed running compileFunction hook: [%s] with error: ',
          ent,
          error
        );
        assert.fail();
  ...
  function assertFunctionRoleName(name: string, roleNameObj: any) {
    ...
    assert.isTrue(
      roleNameObj['Fn::Join'][1].toString().indexOf(name) >= 0,
      'role name contains function name'
    );
  }
  ...
  describe('defaultInherit not set', () => {
    ...
    describe('#validateStatements', () => {
      ...
      it('should throw an error for invalid statement', () => {
        ...
        assert.throws(() => {
          plugin.validateStatements(bad_statement);
        });
      });
    });
    ...
    describe('#getFunctionRoleName', () => {
      ...
      it('should throw an error on long name', () => {
        const long_name =
          'long-long-long-long-long-long-long-long-long-long-long-long-long-name';
        assert.throws(() => {
        ...
        try {
          ...
          assert.isString(msg);
          assert.isTrue(
            msg.startsWith('serverless-iam-roles-per-function: ERROR:')
          );
    ...
    describe('#createRolesPerFunction', () => {
      it('should create role per function', () => {
        ...
        assert.isEmpty(
          helloRole.Properties.ManagedPolicyArns,
          'function resource role has no managed policy'
        );
        //check depends and role is set properlly
        const helloFunctionResource =
          serverless.service.provider.compiledCloudFormationTemplate.Resources
            .HelloLambdaFunction;
        assert.isTrue(
          helloFunctionResource.DependsOn.indexOf(
            'HelloIamRoleLambdaExecution'
          ) >= 0,
        ...
        const helloInheritRole =
          serverless.service.provider.compiledCloudFormationTemplate.Resources
            .HelloInheritIamRoleLambdaExecution;
        assertFunctionRoleName(
          'helloInherit',
          helloInheritRole.Properties.RoleName
        );
        let policy_statements: any[] =
          helloInheritRole.Properties.Policies[0].PolicyDocument.Statement;
        assert.isObject(
          policy_statements.find(
            (s) => s.Action[0] === 'xray:PutTelemetryRecords'
          ),
        ...
        const streamHandlerRole =
          serverless.service.provider.compiledCloudFormationTemplate.Resources
            .StreamHandlerIamRoleLambdaExecution;
        assertFunctionRoleName(
          'streamHandler',
          streamHandlerRole.Properties.RoleName
        );
        policy_statements =
          streamHandlerRole.Properties.Policies[0].PolicyDocument.Statement;
        assert.isObject(
          policy_statements.find(
            (s) =>
              _.isEqual(s.Action, [
        ...
        assert.isObject(
          policy_statements.find((s) => s.Action[0] === 'sns:Publish'),
          'sns dlq statements included'
        );
        const streamMapping =
          serverless.service.provider.compiledCloudFormationTemplate.Resources
            .StreamHandlerEventSourceMappingDynamodbTest;
        assert.equal(
          streamMapping.DependsOn,
          'StreamHandlerIamRoleLambdaExecution'
        );
        //verify sqsHandler should have SQS permissions
        const sqsHandlerRole =
          serverless.service.provider.compiledCloudFormationTemplate.Resources
            .SqsHandlerIamRoleLambdaExecution;
        assertFunctionRoleName(
          'sqsHandler',
          sqsHandlerRole.Properties.RoleName
        );
        policy_statements =
          sqsHandlerRole.Properties.Policies[0].PolicyDocument.Statement;
        ...
        const helloNoPerFunctionResource =
          serverless.service.provider.compiledCloudFormationTemplate.Resources
            .HelloNoPerFunctionLambdaFunction;
        // deprecated
        // assert.isTrue(
        //   helloNoPerFunctionResource.DependsOn.indexOf(
        ...
        assertFunctionRoleName(
          ...
          helloEmptyIamStatementsRole.Properties.RoleName
        );
        ...
        const helloEmptyFunctionResource =
          serverless.service.provider.compiledCloudFormationTemplate.Resources
            .HelloEmptyIamStatementsLambdaFunction;
        assert.isTrue(
          helloEmptyFunctionResource.DependsOn.indexOf(
            'HelloEmptyIamStatementsIamRoleLambdaExecution'
          ) >= 0,
          'function resource depends on role'
        );
      ...
      it('should do nothing when no functions defined', () => {
        ...
        plugin.createRolesPerFunction();
        for (const key in serverless.service.provider
          .compiledCloudFormationTemplate.Resources) {
          if (
            key !== 'IamRoleLambdaExecution' &&
            serverless.service.provider.compiledCloudFormationTemplate.Resources.hasOwnProperty(
              key
            )
          ) {
            const resource =
      ...
      it('should throw when external role is defined', () => {
        _.set(
          serverless.service,
          'functions.hello.role',
          'arn:${AWS::Partition}:iam::0123456789:role/Test'
        );
        assert.throws(() => {
          plugin.createRolesPerFunction();
        });
  ...
  describe('defaultInherit set', () => {
    ...
    beforeEach(() => {
      ...
      _.set(
        serverless.service,
        'custom.serverless-iam-roles-per-function.defaultInherit',
        true
      );
      //change helloInherit to false for testing
      _.set(
        serverless.service,
        'functions.helloInherit.iamRoleStatementsInherit',
        false
      );
      plugin = new Plugin(serverless);
    });
    ...
    describe('#createRolesPerFunction', () => {
      it('should create role per function', () => {
        ...
        let statements: any[] =
          helloRole.Properties.Policies[0].PolicyDocument.Statement;
        assert.isObject(
        ...
        assert.isObject(
          statements.find((s) => s.Action[0] === 'dynamodb:GetItem'),
          'per function statements imported'
        );
        assert.isTrue(
          statements.find((s) => s.Action[0] === 'xray:PutTelemetryRecords') ===
            undefined,
